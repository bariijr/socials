...
<div class="lg:hidden fixed top-0 left-0 right-0 z-30 bg-brand-800 dark:bg-gray-800 shadow-md flex items-center justify-between px-4 h-14">
    ...
    <div class="flex items-center gap-2">
        <a href="/search" class="text-white/80 hover:text-white p-1">
...
<!-- Desktop search bar (above content area) -->
<div class="hidden lg:block lg:pl-64">
    <div class="flex items-center justify-between px-6 py-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        <form method="GET" action="/search" class="flex items-center gap-2 flex-1 max-w-md">
            <div class="relative flex-1">
                <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
                <input type="search" name="q"
                       placeholder="<?= e(__('common.search', 'Tafuta')) ?>..."
                       value="<?= e($_GET['q'] ?? '') ?>"
                       class="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
        </form>
...
<div class="lg:pl-64 flex flex-col min-h-screen">
    <main class="flex-1 pt-14 lg:pt-0 lg:mt-[53px]">
        <!-- Flash messages -->
